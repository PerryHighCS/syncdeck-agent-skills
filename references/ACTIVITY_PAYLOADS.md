...
- **Capability exchange** — `GET /api/syncdeck/embedded-manager-capability?sessionId=<child-session-id>&token=<manager-entry-token>` issues an httpOnly manager capability cookie (`activebits_cap_manager_<base64url(child-session-id)>`) scoped to the child session and returns `{ ok: true }`. No passcode crosses the iframe boundary. This is the activity-agnostic parent-derived handoff from the shared runtime-authentication plan: the endpoint accepts any embedded child that holds a valid one-time entry token minted by its parent, so an activity does not need its own passcode to authenticate an embedded manager through it. Adoption is per-activity, though — a child is capability-authenticated only once its manager client redeems the token via `useEmbeddedManagerCapabilityExchange`. Today that is **Video Sync** only: its embedded manager stays gated until the capability call resolves and its `/manager-access` check re-confirms authority. Credentialless children such as Raffle still ignore the accompanying token (see `buildEmbeddedManagerBootstrapPayload` in `activities/syncdeck/server/routes.ts`) and their embedded manager remains a public principal until that migration happens.
